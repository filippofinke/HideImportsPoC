...
PTIB GetTIB();
PPEB GetPEB();
PLDR_DATA_TABLE_ENTRY GetLdrDataTableEntry(const wchar_t* dllName);
bool LibraryLoaded(const wchar_t* dllName);
void* GetProcAddress(const wchar_t* dllName, const wchar_t* procName);
void* _LoadLibraryW(const wchar_t* dllName);
bool StringContains(const wchar_t* haystack, const wchar_t* needle);
bool StringMatches(const wchar_t* str1, const wchar_t* str2);
int StringLengthW(const wchar_t* str);
int StringLengthA(const char* str);
wchar_t* CharToWChar_T(const char* str);
wchar_t** ParseForwardString(const char* str);
...
*/
void* _LoadLibraryW(const wchar_t* dllName) {
	void* (*__LoadLibraryW)(const wchar_t*);
	__LoadLibraryW = (void* (*)(const wchar_t*))GetProcAddress(L"kernel32.dll", L"LoadLibraryW");
...
*/
void* GetProcAddress(const wchar_t const* dllName, const wchar_t const* procName) {
...
* Return: char
*/
wchar_t ToLowerW(const wchar_t ch) {
	if (ch > 0x40 && ch < 0x5B) {
		return ch + 0x20;
	}
	return ch;
